...
async function init(): Promise<void> {
  ...
  logger.info(`
                                  ...
                                  FSSB TEST VERSION - USE AT YOUR OWN RISK
...
export async function processRaydiumPool(id: PublicKey, poolState: LiquidityStateV4) {
  ...
  if (!quoteMinPoolSizeAmount.isZero()) {
    ...
    // logger.info(`Pool Swap Quote In Amount: ${poolState.swapQuoteInAmount}`);
    // logger.info(`Pool Swap Quote Out Amount: ${poolState.swapQuoteOutAmount}`);
    // logger.info(`Pool Swap Base In Amount: ${poolState.swapBaseInAmount}`);
    // logger.info(`Pool Swap Base Out Amount: ${poolState.swapBaseOutAmount}`);
  }
